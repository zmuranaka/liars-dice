...
public class LiarsDicePlayer
{
    ...
    public int[] firstBet() { return new int[] {1, 1}; }
    ...
    public int[] computerBet(int totalDice, int quantity, int face)
    {
        ...
        {
            if(newQuantity < totalDice) newQuantity++; // Checks if the quantity of dice can be raised
            else newFace++; // The bot will raise the face of the die if they are forced to
        }
        ...
        else
        {
            ...
            else newQuantity++; // The bot will resort to raising the quantity of the dice if the face is 6
        }        
        return new int[] {newQuantity, newFace};
    }

    // Returns who the current player is
    public String getCurrentPlayer()
    {
        if(id == 1) return "Player 1";
        else if(id == 2) return "Player 2";
        else if(id == 3) return "Player 3";
        else return "You";
    }
    ...
    {
        return newQuantity > oldQuantity || newFace > oldFace;
    }
    ...
    {
        return quantity > totalDice || 0 > quantity || face > 6 || 1 > face;
    }
    ...
    public boolean betIsUnlikely(int totalDice, int quantity, int face)
    {
        ...
        if(face == 1) expectedQuantity = (int)(totalDice / 6);
        else expectedQuantity = (int)(totalDice / 3);
        int reasonableVariance = (int)(expectedQuantity + (0.2 * totalDice));
        
        return quantity > reasonableVariance; // The bet is unlikely if the quantity is higher than reasonable variance
    }
}
